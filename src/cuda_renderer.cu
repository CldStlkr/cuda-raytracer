...
__global__ __launch_bounds__(256, 2) void generate_rays(PathStateSOA paths, camera_gpu cam, curandState* rand_state,
...
__device__ float perlin_interp(const vec3_gpu c[2][2][2], float u, float v, float w) {
  ...
  for (int i = 0; i < 2; i++)
    for (int j = 0; j < 2; j++)
      for (int k = 0; k < 2; k++) {
        ...
        accum += (i * uu + (1.0f - i) * (1.0f - uu)) * (j * vv + (1.0f - j) * (1.0f - vv)) *
  ...
  for (int di = 0; di < 2; di++)
    for (int dj = 0; dj < 2; dj++)
      for (int dk = 0; dk < 2; dk++) {
        int idx = pdata.perm_x[(i + di) & 255] ^ pdata.perm_y[(j + dj) & 255] ^ pdata.perm_z[(k + dk) & 255];
        c[di][dj][dk] = to_gpu(pdata.randvec[idx]);
      }
  return perlin_interp(c, u, v, w);
}

__device__ float perlin_turb(const point3_gpu& p, const PerlinDataGPU& pdata, int depth) {
  float accum = 0.0f;
  point3_gpu temp_p = p;
  float weight = 1.0f;
...
__device__ vec3_gpu get_texture_color(const TextureGPU& tex, float u, float v, const point3_gpu& p,
  ...
  if (tex.type == TextureType::SOLID) {
    ...
    return vec3_gpu(1, 1, 1) * 0.5f *
           (1.0f + sinf(tex.noise.scale * p.z() + 10.0f * perlin_turb(p, perlin[tex.noise.perlin_data_idx], 7)));
  }
...
__global__ void shade_kernel(PathStateSOA paths, HitResultSOA hits, int* active_indices, int num_active,
  ...
  if (hits.hit_anything[idx]) {
    ...
    if (mat.type == MaterialType::DIFFUSE_LIGHT) {
      paths.color[path_idx] += paths.attenuation[path_idx] *
                               get_texture_color(textures[mat.albedo_tex_id], rec_u, rec_v, rec_p, perlin, images);
    ...
    if (mat.type == MaterialType::LAMBERTIAN) {
      ...
      else direction = refract(unit_direction, rec_normal, refraction_ratio);
...
__device__ vec3_gpu aces_tonemap(vec3_gpu x) {
  ...
  return vec3_gpu(fmaxf(0.0f, fminf(1.0f, res.x())), fmaxf(0.0f, fminf(1.0f, res.y())),
                  fmaxf(0.0f, fminf(1.0f, res.z())));
}
...
__global__ void finalize(float4* frame_buffer, vec3_gpu* accum_buffer, int total_pixels, int total_samples) {
  ...
  frame_buffer[idx] = make_float4(tonemapped.x(), tonemapped.y(), tonemapped.z(), 1.0f);
}
...
extern "C" void launch_render(RenderConfig config, span<LinearBVHNode> h_bvh, span<PrimitiveGPU> h_prims,
  ...
  cam.lookfrom = to_gpu(config.lookfrom);
  cam.lookat = to_gpu(config.lookat);
  cam.vup = to_gpu(config.vup);
  cam.vfov = config.vfov;
  cam.defocus_angle = config.defocus_angle;
  cam.focus_dist = config.focus_dist;
  cam.background = to_gpu(config.background);
  cam.initialize();
  ...
  cudaMalloc(&d_active, total_rays * sizeof(int));
  ...
  finalize<<<(width * height + 255) / 256, 256>>>((float4*)config.frame_buffer, d_accum, width * height,
  ...
  cudaFree(d_prims);
  cudaFree(d_mats);
  if (d_texs) cudaFree(d_texs);
  if (d_perlin) cudaFree(d_perlin);
  if (d_imgs) cudaFree(d_imgs);
}
...
extern "C" void* import_vulkan_memory(int fd, size_t size) {
  if (s_extMem) {
    cudaDestroyExternalMemory(s_extMem);
    s_extMem = nullptr;
  }
  ...
  bd.size = size;
  void* p;
  if (cudaExternalMemoryGetMappedBuffer(&p, s_extMem, &bd) != cudaSuccess) return nullptr;
  return p;
}
