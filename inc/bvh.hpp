...
public:
  ...
  std::shared_ptr<hittable> left() { return left_; }
  std::shared_ptr<hittable> right() { return right_; }
...
// Out-of-line definition — bvh_node is now complete, so make_shared<bvh_node> works
inline bvh_node::bvh_node(std::vector<shared_ptr<hittable>>& objects, size_t start, size_t end) {
  bbox_ = aabb::empty;
